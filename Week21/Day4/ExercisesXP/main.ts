...
class Product {
  ...
  getProductInfo() {
    return `This is ${this.name} and it costs \$${this.price}`;
  }
}

const pc = new Product("Lenovo PC", 10000);
console.log(pc.getProductInfo());
pc.id = 5; //compiler shows error
...
console.log(Calculator.add(1, 2), Calculator.subtract(1, 2));
...
function printUserDetails(user: PremiumUser) {
  console.log(
    `Name: ${user.name}; Email: ${user.email}; ${user.membershipLevel !== undefined ? `Membership Level: ${user.membershipLevel};` : ""}`,
  );
}
