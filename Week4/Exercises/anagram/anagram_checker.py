...
class AnagramChecker:
    def __init__(self, words_file="./sowpods.txt") -> None:
        self.__words = open(words_file, "r").read().split()

    def is_valid_word(self, word):
        return word.upper() in self.__words

    @staticmethod
    def __count_letters(word):
        count = {}
        for c in word.lower():
            if c in count:
                count[c] += 1
    ...
    @classmethod
    def __is_anagram(cls, word1, word2):
        return word1.lower() != word2.lower() and cls.__count_letters(
            word1
        ) == cls.__count_letters(word2)

    def get_anagrams(self, word):
        anagrams = []
        for w in self.__words:
            if self.__is_anagram(w, word):
