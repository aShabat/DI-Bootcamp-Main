...
search.term.addEventListener("input", refreshRobots);
...
function refreshRobots() {
  ...
  robots
    .filter(({ name }) => name.toLowerCase().match(term) !== null)
    .forEach((r) => robotsDiv.appendChild(robotIcon(r)));
}

refreshRobots();

function robotIcon(robot) {
